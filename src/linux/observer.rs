...
impl LinuxObserver {
	pub(crate) fn new(
		...
		let (conn, screen_id) = x11rb::connect(None)
			.map_err(|e| format!("Failed to connect to the x11 server: {e}"))?;
...
impl LinuxObserver {
	...
	fn get_clipboard_content(&mut self) -> Result<Option<Body>, ErrorWrapper> {
		...
		if formats.contains_id(self.x11.atoms.PNG_MIME) {
			let bytes = self.x11.extract_clipboard_content(
			...
			let bytes = self
				.x11
				.extract_clipboard_content(format, &formats, None)?;
...
impl LinuxObserver {
	fn resolve_atom_names(&mut self, atoms: &[Atom]) -> Result<Formats, ErrorWrapper> {
		...
		for (atom, cookie) in cookies {
			...
			let name: Arc<str> = String::from_utf8_lossy(&reply.name)
				.as_ref()
				.into();
	...
	fn extract_file_list(&self, available_formats: &Formats) -> Result<Vec<PathBuf>, ErrorWrapper> {
		let raw_data = self.x11.extract_clipboard_content(
			self.x11.atoms.FILE_LIST,
			available_formats,
			None,
		)?;
		...
		if initial_reply.type_ == self.atoms.INCR {
			...
			loop {
				...
				let event = self
					.conn
					.poll_for_event()
					.map_err(to_read_error)?; // Don't need sequence number here
